...
namespace JUST.Shared.DatabaseRepository
{
    public class DatabaseRepository : IDatabaseRepository
    {
        ...
        private string AgedReceivablesQueryString = "select invoice.cusnum customerNumber, customer.name customerName, invoice.invnum invoiceNumber, invoice.jobnum jobNumber, invoice.sourceid workOrderNumber, invoice.invdate invoiceDate, invoice.duedate duedate, round((invoice.grossamt + invoice.applied1 + invoice.applied2 + invoice.applied3 + invoice.applied4), 2) amountDue, invoice.memo memo from invoice inner join customer on customer.cusnum = invoice.cusnum where round((invoice.grossamt + invoice.applied1 + invoice.applied2 + invoice.applied3 + invoice.applied4), 2) > 0 order by invoice.cusnum";
        ...
        public List<AgedReceivable> GetAgedReceivables()
        {
            ...
            var dueDateColumn = reader.GetOrdinal("dueDate");
            ...
            while (reader.Read())
            {
                ...
                var dueDate = reader.GetDate(dueDateColumn);
                var amountDue = reader.GetDecimal(amountDueColumn);
                var memo = reader.GetString(memoColumn);

                agedRecevables.Add(new AgedReceivable(customerNumber, customerName, invoiceNumber, invoiceDate, dueDate, jobNumber, workOrderNumber, amountDue, memo));
            }
